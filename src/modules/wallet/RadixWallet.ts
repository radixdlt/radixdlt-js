...
export class RadixWallet extends events.EventEmitter {
    ...
    openNodeConnection = async () => {
        ...
        try {
            ...
            //Subscribe to events
            this.nodeConnection.subscribe(this.keyPair, this._onAtomReceived, this._onConnectionClosed) 
    ...
    async sendTransaction(address: string, decimalQuantity: number, asset_id: string, message?: string) {
        ...
        for (let [id, consumable] of this.unspentConsumables.entries()) {
            if ((consumable as RadixConsumable).asset_id.toString() != asset_id) {continue}
    ...
    async sendApplicationMessage(to: Array<string>, payload: any, applicationId: string, encrypted = true) {
        const recipients = []
        ...
        let atom = RadixApplicationPayloadAtom.withEncryptedPayload(payload, recipients, applicationId, encrypted)
    ...
    private _addAtomToTransactionList = async (atom: RadixTransactionAtom, notify: boolean = true) => {
        ...
        for (const particle of atom.particles as Array<RadixConsumer|RadixConsumable|RadixEmission>) {
            ...
            if (!(asset_id in this.assets)) {
                this.assets[asset_id] = await this.nodeConnection.getAtomById(new RadixEUID(asset_id))
            }
            let asset = this.assets[asset_id]
            ...
            if (ownedByMe && !isFee) {
                ...
                } else if (particle.serializer === RadixConsumable.SERIALIZER || particle.serializer === RadixEmission.SERIALIZER) {
                    quantity += particle.quantity
    ...
    private _addAtomToMessageList(atom: RadixApplicationPayloadAtom, notify: boolean = true) {
        //Format message
    ...
    private _addAtomToApplicationMessages(atom: RadixApplicationPayloadAtom) {
        ...
        this.applicationMessages.get(applicationMessage.applicationId).push(applicationMessage)
    ...
    public addContact(address: string | RadixKeyPair | RadixECKeyPair, alias: string = null) {
    ...
    getApplicationMessages(applicationId: string): Observable<{applicationId: string, payload: any}> {
        return Observable.create((observer: Observer<{applicationId: string, payload: any}>) => {
            ...
            // Subscribe for new ones
            this.applicationMessageSubject.filter((applicationMessage) => {return applicationMessage.applicationId === applicationId})
        })
    }
